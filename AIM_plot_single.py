...
def get_vehicle_type(information_address):
    ...
    df_T = pd.DataFrame(df_vehicle_information)
    # print(df_T)
    vehicle_id = 0
    vehicle_total_list = []
    car_total_list = []
...
def draw_trajectory(x, y1_car, y1_bus, y2_car, y2_bus, r1, r2, mode):
    ...
    if mode == 0:
        ...
        plt.axhline(y=conflict_in_r1,  # 线高
                    ...
                    xmax=X[-1],  # 线结束位置
                    color="green",
                    label=r"$i_{"+str(order_r1)+","+str(r1)+"}$",
                    linestyle='--',  # 线型
                    linewidth=0.5,  # 线宽
                    # marker='*',#线两端marker
                    markerfacecolor='w',
        ...
        plt.axhline(y=conflict_out_r1,  # 线高
                    ...
                    xmax=X[-1],  # 线结束位置
                    color="green",
                    label=r"$o_{"+str(order_r1)+","+str(r1)+"}$",
                    linestyle='-.',  # 线型
                    linewidth=0.5,  # 线宽
                    # marker='*',#线两端marker
                    markerfacecolor='w',
        ...
        plt.axhline(y=conflict_in_r2,  # 线高
                    ...
                    xmax=X[-1],  # 线结束位置
                    color="red",
                    label=r"$i_{"+str(order_r2)+","+str(r2)+"}$",
                    linestyle='--',  # 线型
                    linewidth=0.5,  # 线宽
                    # marker='*',#线两端marker
                    markerfacecolor='w',
        ...
        plt.axhline(y=conflict_out_r2,  # 线高
                    ...
                    xmax=X[-1],  # 线结束位置
                    color="red",
                    label=r"$o_{"+str(order_r2)+","+str(r2)+"}$",
                    linestyle='-.',  # 线型
                    linewidth=0.5,  # 线宽
                    # marker='*',#线两端marker
                    markerfacecolor='w',
...
def test():
    while True:
        ...
        vehicle_information_address = "/Users/fyc/PycharmProjects/AIM/旧的结果/car_information.xlsx"
        vehicle_time_table_address = "/Users/fyc/PycharmProjects/AIM/数据/" + lab_name + '/' + "/car_time" + str(bus_mode) + ".csv"
        vehicle_position_address = "/Users/fyc/PycharmProjects/AIM/数据/" + lab_name + '/' + "/car_position" + str(bus_mode) + ".csv"
        ...
        vehicle_total_list, car_total_list, bus_total_list = get_vehicle_type(vehicle_information_address)
        ...
        fig.savefig('/Users/fyc/PycharmProjects/AIM/数据/'+str(lab_name)+"/公交车行驶时间"+str(bus_mode)+".svg", format='svg')
        fig.show()
        ...
        fig.savefig('/Users/fyc/PycharmProjects/AIM/数据/'+str(lab_name)+"/车辆轨迹"+str(bus_mode)+".svg", format='svg')
        fig.show()
