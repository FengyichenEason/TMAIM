...
slight, elight = AIM_tolls.make_traffic_light_table(total_time=total_time, cycle_time=C)
...
def AIM_problem(mode,
    ...
    if mode == 0:
        ...
        MODEL.addConstrs((y_in[b, c, t] - 1) * M <= x[c, t] - car_intersection_conflict[c][b][2]
                         for c in range(car) for t in range(total_time) for b in range(intersection_conflict_num))
        MODEL.addConstrs(
            y_out[b, c, t] * M >= car_intersection_conflict[c][b][3] + car_type[c] * l_c + (1 - car_type[c]) * l_b - x[
                c, t]
            for c in range(car) for t in range(total_time) for b in range(intersection_conflict_num))
        MODEL.addConstrs((y_out[b, c, t] - 1) * M <= car_intersection_conflict[c][b][3] + car_type[c] * l_c + (
                    1 - car_type[c]) * l_b - x[c, t]
                         for c in range(car) for t in range(total_time) for b in range(intersection_conflict_num))
...
def main():
    ...
    plist = [0.3, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3]
    time = 50
    ...
    if done == 1:
        for experiment_num in range(6):
            ...
            flow = list(flow)
            flow = AIM_tolls.add_bus_flows(flow, 2 * 1, 8)
            car_information_list[1] = flow
            df = pd.DataFrame(car_information_list)
            df.to_csv("C:\\Users\\15470\PycharmProjects\IntersectionControl\AIM\数据\实验" + str(experiment_num + 1) + ".csv")
    if mode == 0 and consider == 0:
        base_address = "C:\\Users\\15470\PycharmProjects\IntersectionControl\AIM\数据\车流密度变化\无信号不考虑公平"
    if mode == 0 and consider == 1:
        base_address = "C:\\Users\\15470\PycharmProjects\IntersectionControl\AIM\数据\车流密度变化\无信号考虑公平"
    if mode == 1 and consider == 0:
        base_address = "C:\\Users\\15470\PycharmProjects\IntersectionControl\AIM\数据\车流密度变化\信号不考虑公平"
    if mode == 1 and consider == 1:
        base_address = "C:\\Users\\15470\PycharmProjects\IntersectionControl\AIM\数据\车流密度变化\信号考虑公平"
    for experiment_num in range(5, 6):
        with PrintToFile(base_address + '\实验' + str(experiment_num + 1) + '\日志' + str(experiment_num + 1) + '.txt'):
            print("实验" + str(experiment_num + 1))
            df = pd.read_csv("C:\\Users\\15470\PycharmProjects\IntersectionControl\AIM\数据\实验" + str(experiment_num + 1) + ".csv")
